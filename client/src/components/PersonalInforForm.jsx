...
const PersonalInforForm = ({
  ...
  return (
    <div>
      <h3 className="lext-lg font-semibold text-gray-900">
      ...
      {fields.map((field) => {
        ...
        return (
            <div key={field.key} className="space-y-1 mt-5" >
                ...
                <input type={field.type} value={data[field.key] || ""} onChange={(e) => handleChange(field.key, e.target.value)} className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500 focus:boder-blue-500 outline-none transition-colors text-sm" placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required} />
            </div>
        )
      })}
    </div>
